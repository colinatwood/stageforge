...
bool valid_sha256_token(std::string_view token) noexcept {
...
struct NativeMidiInput::Impl {
    ...
    void push(std::uint8_t byte) noexcept {
        ...
        if (next == read.load(std::memory_order_acquire)) { drops.fetch_add(1, std::memory_order_relaxed); return; }
        ring[current] = byte;
    ...
    static void CALLBACK callback(HMIDIIN, UINT message, DWORD_PTR instance, DWORD_PTR first, DWORD_PTR) noexcept {
        auto* self = reinterpret_cast<Impl*>(instance); if (!self) return;
        if (message == MIM_CLOSE || message == MIM_ERROR || message == MIM_LONGERROR) { self->revoked.store(true, std::memory_order_release); return; }
        if (message != MIM_DATA || self->revoked.load(std::memory_order_acquire)) return;
        const auto packed = static_cast<std::uint32_t>(first);
        const auto status = static_cast<std::uint8_t>(packed & 0xffU); self->push(status);
    ...
    static void notify(const MIDINotification* notification, void* context) noexcept {
        ...
        if (selected != 0 && static_cast<std::uintptr_t>(removed->child) == selected) self->revoked.store(true, std::memory_order_release);
    }
    static void callback(const MIDIPacketList* packets, void* context, void*) noexcept {
        ...
        for (UInt32 p = 0; p < packets->numPackets; ++p) { for (UInt16 i = 0; i < packet->length; ++i) self->push(packet->data[i]); packet = MIDIPacketNext(packet); }
    }
#endif
};
...
bool NativeMidiInput::attach(std::string_view native_hash) noexcept {
    detach();
    if (!valid_sha256_token(native_hash)) return false;
    impl_->read.store(0, std::memory_order_relaxed); impl_->write.store(0, std::memory_order_relaxed); impl_->drops.store(0, std::memory_order_relaxed);
#ifdef _WIN32
    impl_->revoked.store(false, std::memory_order_relaxed);
    const auto count = midiInGetNumDevs();
    for (UINT index = 0; index < count; ++index) {
        ULONG bytes = 0; std::string interface_name;
        const auto query = [&](UINT msg, DWORD_PTR first, DWORD_PTR second) { return midiInMessage(reinterpret_cast<HMIDIIN>(static_cast<UINT_PTR>(index)), msg, first, second); };
        if (query(DRV_QUERYDEVICEINTERFACESIZE, reinterpret_cast<DWORD_PTR>(&bytes), 0) == MMSYSERR_NOERROR && bytes >= sizeof(wchar_t) && bytes <= 65536 && bytes % sizeof(wchar_t) == 0) {
            ...
            if (query(DRV_QUERYDEVICEINTERFACE, reinterpret_cast<DWORD_PTR>(buffer.data()), bytes) == MMSYSERR_NOERROR && buffer.back() == L'\0') {
                ...
                if (needed > 1) { std::string text(static_cast<std::size_t>(needed), '\0'); if (WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, buffer.data(), -1, text.data(), needed, nullptr, nullptr)) { text.pop_back(); interface_name = std::move(text); } }
            }
        }
        if (interface_name.empty()) continue;
        const auto candidate = sha256_token("winmm-native:input:" + interface_name + ":" + std::to_string(index)); if (candidate != native_hash) continue;
        ...
        impl_->input = handle; if (midiInStart(handle) != MMSYSERR_NOERROR) { midiInClose(handle); impl_->input = nullptr; return false; } return true;
    }
#elif defined(__APPLE__)
    impl_->revoked.store(false, std::memory_order_relaxed); impl_->selected_source.store(0, std::memory_order_relaxed);
    ...
    for (ItemCount index = 0; index < count; ++index) {
        const auto source = MIDIGetSource(index); if (!source) continue;
        const auto candidate = sha256_token("coremidi-native:" + std::to_string(static_cast<std::uint64_t>(source)) + ":in"); if (candidate != native_hash) continue;
        if (MIDIPortConnectSource(impl_->port, source, nullptr) != noErr) { detach(); return false; }
        impl_->source = source; impl_->selected_source.store(static_cast<std::uintptr_t>(source), std::memory_order_release); return true;
    }
    detach();
#endif
    return false;
...
void NativeMidiInput::detach() noexcept {
    ...
    impl_->revoked.store(false, std::memory_order_relaxed);
#elif defined(__APPLE__)
    impl_->revoked.store(true, std::memory_order_release); impl_->selected_source.store(0, std::memory_order_release);
    if (impl_->port && impl_->source) MIDIPortDisconnectSource(impl_->port, impl_->source); impl_->source = 0;
...
std::size_t NativeMidiInput::poll_bytes(std::uint8_t* destination, std::size_t capacity) noexcept {
    ...
    std::size_t count = 0; auto current = impl_->read.load(std::memory_order_relaxed); const auto end = impl_->write.load(std::memory_order_acquire);
    while (current != end && count < capacity) { destination[count++] = impl_->ring[current]; current = (current + 1) % ring_capacity; }
    impl_->read.store(current, std::memory_order_release); return count;
}
