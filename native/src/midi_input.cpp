...
#if defined(__linux__)
bool is_midi_path(const char* name) noexcept {
    return name && std::strncmp(name, "midiC", 5) == 0 && std::strchr(name, 'D') != nullptr;
}
...
std::string_view native_hash_from_identity(std::string_view identity) noexcept {
    const auto marker=identity.find(":hash:"); if(marker==std::string_view::npos) return {}; return identity.substr(marker+6);
}
#endif
} // namespace

std::uint8_t MidiByteParser::data_length(std::uint8_t status) noexcept {
    if (status < 0x80) return 0; if (status < 0xF0) { const auto type=status&0xF0; return (type==0xC0||type==0xD0)?1:2; }
    switch(status){case 0xF1:case 0xF3:return 1;case 0xF2:return 2;default:return 0;}
}
...
bool MidiByteParser::feed(std::uint8_t byte,std::uint64_t show_time_ns,MidiInputMessage& out) noexcept {
    ...
    if(byte&0x80){if(byte==0xF0){in_sysex_=true;running_status_=message_status_=expected_=received_=0;return false;} if(byte==0xF7)return false;
        ...
        if(expected_==0){out={show_time_ns,byte,0,0};return true;} return false;}
    if(expected_==0){if(running_status_==0)return false;message_status_=running_status_;expected_=data_length(running_status_);received_=0;}
    if(received_<data_.size())data_[received_]=byte; ++received_; if(received_<expected_)return false;
    out={show_time_ns,message_status_,data_[0],expected_>1?data_[1]:std::uint8_t{0}}; received_=0; if(message_status_>=0xF0){expected_=0;message_status_=0;} return true;
}
...
bool MidiInputManager::attach(std::string_view device_id,std::string_view player_id) noexcept {
    ...
    std::unique_ptr<NativeMidiInput, NativeMidiInputDeleter> native(new (std::nothrow) NativeMidiInput()); if (!native || !native->attach(native_hash)) return false;
