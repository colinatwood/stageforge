...
def windows_verify_and_launch(executable:Path,argv:Sequence[str]=(),*,expected_adapter_sha256:str|None=None,expected_publisher_certificate_sha256:str|None=None,env:Mapping[str,str]|None=None)->tuple[BoundProcess,dict]:
    ...
    kernel32.GetFileInformationByHandle.argtypes=[ctypes.c_void_p,ctypes.POINTER(BY_HANDLE_FILE_INFORMATION)];kernel32.GetFileInformationByHandle.restype=ctypes.c_int
    ...
    if handle in {None,INVALID_HANDLE_VALUE}:raise PlatformLaunchBindingError("could not lock adapter executable for verified launch")
    try:
        info=BY_HANDLE_FILE_INFORMATION()
        if not kernel32.GetFileInformationByHandle(ctypes.c_void_p(handle),ctypes.byref(info)):raise PlatformLaunchBindingError("could not read adapter Windows file identity")
        with path.open("rb") as stream:digest=_sha256_stream(stream)
        ...
        if signature["signatureStatus"]!="Valid":raise PlatformLaunchBindingError("adapter Authenticode signature is not valid")
...
def _codesign_evidence(path:Path)->dict:
    ...
    if team_id.lower()=="not set":team_id=""
...
def macos_verify_and_launch(executable:Path,argv:Sequence[str]=(),*,expected_adapter_sha256:str|None=None,expected_team_id:str|None=None,expected_code_directory_hash:str|None=None,env:Mapping[str,str]|None=None)->tuple[BoundProcess,dict]:
    ...
    try:
        ...
        staged.chmod(0o700)
        with staged.open("rb") as stream:staged_digest=_sha256_stream(stream)
        if staged_digest!=digest:raise PlatformLaunchBindingError("private staged adapter copy hash mismatch")
        signature=_codesign_evidence(staged)
        if expected_team_id:
            wanted=expected_team_id.strip().upper()
            if not _TEAM.fullmatch(wanted) or signature["teamId"]!=wanted:raise PlatformLaunchBindingError("adapter macOS Team ID mismatch")
