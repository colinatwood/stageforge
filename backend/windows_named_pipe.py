...
def expected_windows_pipe_sids(allowed_sids: Iterable[str], *, allow_administrators: bool = False) -> tuple[str, ...]:
    ...
    for value in allowed_sids:
        ...
        if sid not in seen:
            normalized.append(sid);seen.add(sid)
    if not normalized:
        raise ValueError("at least one explicit service/operator SID is required")
    values=["S-1-5-18"]
    if allow_administrators: values.append("S-1-5-32-544")
...
def validate_windows_pipe_acl_facts(policy: "WindowsPipePolicy", facts: dict) -> dict:
    ...
    return {"daclProtected": True, "allowedSids": sorted(actual), "accessMasks": {sid:_GENERIC_ALL for sid in sorted(actual)}}
...
class _Win32Api:
    def __init__(self) -> None:
        ...
        self.advapi32.ConvertStringSecurityDescriptorToSecurityDescriptorW.argtypes = [ctypes.c_wchar_p, ctypes.c_ulong, ctypes.POINTER(ctypes.c_void_p), ctypes.POINTER(ctypes.c_ulong)]
        self.advapi32.ConvertStringSecurityDescriptorToSecurityDescriptorW.restype = ctypes.c_int
        self.kernel32.CreateNamedPipeW.argtypes = [ctypes.c_wchar_p, ctypes.c_ulong, ctypes.c_ulong, ctypes.c_ulong, ctypes.c_ulong, ctypes.c_ulong, ctypes.c_ulong, ctypes.c_void_p]
        ...
        self.advapi32.GetSecurityInfo.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.c_ulong, ctypes.c_void_p, ctypes.c_void_p, ctypes.POINTER(ctypes.c_void_p), ctypes.c_void_p, ctypes.POINTER(ctypes.c_void_p)]
        ...
        self.kernel32.GetAclInformation.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_ulong, ctypes.c_int]
        self.kernel32.GetAclInformation.restype = ctypes.c_int
        self.kernel32.GetAce.argtypes = [ctypes.c_void_p, ctypes.c_ulong, ctypes.POINTER(ctypes.c_void_p)]
        self.kernel32.GetAce.restype = ctypes.c_int
...
def inspect_windows_pipe_dacl(handle: int, api: _Win32Api) -> dict:
    ...
    result=api.advapi32.GetSecurityInfo(ctypes.c_void_p(handle),_SE_KERNEL_OBJECT,_DACL_SECURITY_INFORMATION,None,None,ctypes.byref(dacl),None,ctypes.byref(descriptor))
    if result != 0 or not descriptor.value:
        raise WindowsNamedPipeError(f"could not inspect Windows named-pipe DACL ({result})")
    try:
        control=ctypes.c_ushort(0);revision=ctypes.c_ulong(0)
        ...
        if not dacl.value:
            return {"daclProtected":bool(control.value&_SE_DACL_PROTECTED),"nullDacl":True,"allowedSids":[],"accessMasks":{},"unsupportedAceCount":0}
        info=ACL_SIZE_INFORMATION()
        if not api.kernel32.GetAclInformation(dacl,ctypes.byref(info),ctypes.sizeof(info),_ACL_SIZE_INFORMATION):
            raise WindowsNamedPipeError(f"could not inspect Windows named-pipe ACL ({api.last_error()})")
        allowed=[];masks={};unsupported=0
        for index in range(int(info.AceCount)):
            ace_ptr=ctypes.c_void_p()
            if not api.kernel32.GetAce(dacl,index,ctypes.byref(ace_ptr)) or not ace_ptr.value:
            ...
            try: sid=sid_text.value.upper()
            finally: api.kernel32.LocalFree(ctypes.cast(sid_text,ctypes.c_void_p))
            allowed.append(sid);masks[sid]=int(ace.Mask)
        return {"daclProtected":bool(control.value&_SE_DACL_PROTECTED),"nullDacl":False,"allowedSids":allowed,"accessMasks":masks,"unsupportedAceCount":unsupported}
    finally:
...
class Win32MessagePipeConnection:
    ...
    def close(self) -> None:
        ...
        self._closed = True
        try:self._api.kernel32.FlushFileBuffers(ctypes.c_void_p(self._handle))
        finally:
...
class Win32NamedPipeListener:
    ...
    def __init__(self, name: str, policy: WindowsPipePolicy, *, api: _Win32Api | None = None) -> None:
        self.name = validate_windows_pipe_name(name);self.policy = policy;self._api = api or _Win32Api();self._closed = False
        self.security_descriptor_sddl = policy.sddl()

    def _security_attributes(self):
        descriptor = ctypes.c_void_p()
        ok = self._api.advapi32.ConvertStringSecurityDescriptorToSecurityDescriptorW(ctypes.c_wchar_p(self.security_descriptor_sddl),ctypes.c_ulong(_SDDL_REVISION_1),ctypes.byref(descriptor),None)
    ...
    def accept(self) -> Win32MessagePipeConnection:
        if self._closed:raise WindowsNamedPipeError("Windows named-pipe listener is closed")
        descriptor, attrs = self._security_attributes()
        try:
            handle = self._api.kernel32.CreateNamedPipeW(ctypes.c_wchar_p(self.name),ctypes.c_ulong(_PIPE_ACCESS_DUPLEX | _FILE_FLAG_FIRST_PIPE_INSTANCE),ctypes.c_ulong(_PIPE_TYPE_MESSAGE | _PIPE_READMODE_MESSAGE | _PIPE_WAIT),ctypes.c_ulong(1),ctypes.c_ulong(MAX_FRAME + _SIZE.size),ctypes.c_ulong(MAX_FRAME + _SIZE.size),ctypes.c_ulong(5000),ctypes.byref(attrs))
            ...
            try:attest_windows_pipe_dacl(int(handle),self.policy,self._api)
            except Exception:
                self._api.kernel32.CloseHandle(ctypes.c_void_p(handle));raise
        ...
        finally:self._api.kernel32.LocalFree(descriptor)

    def close(self) -> None:self._closed = True


def create_secure_windows_pipe_listener(name: str,allowed_sids: Iterable[str],*,allow_administrators: bool = False,api: _Win32Api | None = None) -> Win32NamedPipeListener:
